# -*- coding: utf-8 -*-
"""phase5 传送门验证 v2: 裂影渊 -> 泣魔渊

v2 改动:
  - crop 放宽到 120 (识别完整 3 字地图名)
  - 点满 3 次后不立即退出, 继续等传送动画(最多 10s)检测地图切换
  - verbose 打印 OCR 原始结果(含低 conf), 排查识别异常
"""
...
def main():
    ...
    try:
        while True:
            ...
            if clicks < MAX_CLICKS and time.time() - last_click >= INTERVAL:
                tap(*PORTAL_POS)
                clicks += 1
                last_click = time.time()
                log(f"传送门点击 {clicks}/{MAX_CLICKS} @{PORTAL_POS}")
                get_map_name(verbose=True)
                if clicks >= MAX_CLICKS:
                    done_deadline = time.time() + DONE_WAIT
                    log(f"点满 {MAX_CLICKS} 次，继续等传送动画 {DONE_WAIT}s...")
