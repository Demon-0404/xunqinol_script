# -*- coding: utf-8 -*-
"""铁1(赤炼) Phase 8 调试：右下角3次→右上角3次，检测进入新地图"""
...
def walk_phase_change(desc, pos, times=3):
    print(f"── {desc} 点 {times} 次 ──")
    original = get_map_name()
    print(f"  初始地图: '{original}'")
    for i in range(times):
        ...
        name = get_map_name()
        changed = bool(name) and name != original
        tag = "  [地图已变!]" if changed else ""
...
print("=== 铁1 Phase 8: 右下角3次→右上角3次，检测进入新地图 ===")
if not walk_phase_change("右下角", RIGHT_DOWN, 3):
    walk_phase_change("右上角", RIGHT_UP, 3)
print("=== Phase 8 完成 ===")
