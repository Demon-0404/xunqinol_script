# -*- coding: utf-8 -*-
"""铁2(浮游) Phase 6 调试：回忆之地 自动遇怪2次→找红衣女→提交→接取"""
...
def scan_npc_page():
    ...
    try:
        Image.fromarray(arr).save(os.path.join(LOG_DIR, f"_debug_tie2_npc_{ts}.png"))
    except Exception:
        pass
...
def auto_battle_phase(count=BATTLE_COUNT):
    ...
    while battles < count and time.time() - t0 < 300:
        ...
        time.sleep(0.3)
    # 取消前必须确保已离开战斗，否则按键0落在战斗中无效，自动遇怪继续打断后续寻路
    while is_in_battle():
        print("  取消前仍在战斗中，等待本场结束...")
        wait_battle_end()
    print(f"  按键0 取消自动遇怪 (共{battles}场)")
    tap(KEY0, "键0", wait=0.5)
    time.sleep(1.0)
    if is_in_battle():
        print("  取消后仍检测到战斗，等待结束...")
        wait_battle_end()
...
print("=== 铁2 Phase 6: 回忆之地 遇怪2次→找红衣女→提交→接取 ===")
auto_battle_phase()
