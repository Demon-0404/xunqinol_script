# -*- coding: utf-8 -*-
"""铁4(魔界之门) Phase 0 调试：备忘→副本→OCR找魔界→点行→瞬间传送"""
...
print("=== 铁4 Phase 0: 备忘→副本→OCR找魔界→点行→瞬间传送 ===")

print("步骤1: 点击备忘 (1000,1200)")
tap(MEMO, wait=1.2)

print("步骤2: 点击副本标签 (750,200)")
tap(DUNGEON_TAB, wait=1.2)

print("步骤3: 截图 + OCR 识别副本列表")
arr = screenshot()
dump_text(arr)

pos = find_text(arr, "魔界", 300, 1300) or find_text(arr, "魔", 300, 1300)
if not pos:
    print("!!! 未找到 魔界，请查看上方识别结果")
    sys.exit(1)
cx, cy, text, conf = pos
