# -*- coding: utf-8 -*-
"""铁4(魔界之门) Phase 6 调试：右下角3次→右上角3次回隐之境，并计数战斗"""
...
TARGET_KEY = "隐"      # 段1目标：回到隐之境
TARGET_KEY2 = "炼魂"   # 段2目标：进入炼魂祭坛
...
def wait_battle_end(is_boss=False):
    ...
    if not is_boss:
        battle_count += 1
        print(f"  战斗结束! 累计 {battle_count} 场")
    time.sleep(12.0 if is_boss else 1.0)


def walk_phase(desc, pos, target_key, times=3):
...
print("=== 铁4 Phase 6: 回隐之境 → 进炼魂祭坛，计数战斗 ===")
print(f"  初始地图: '{get_map_name()}'")
# 段1: 鬼爪炼狱 → 隐之境
if TARGET_KEY not in get_map_name():
    if not walk_phase("右下角(1050,1100)", RIGHT_DOWN, TARGET_KEY, 3):
        walk_phase("右上角(1050,400)", RIGHT_UP, TARGET_KEY, 3)
print(f"  段1结束地图: '{get_map_name()}'")
# 段2: 隐之境 → 炼魂祭坛 (点击(1050,650)3次)
if TARGET_KEY2 not in get_map_name():
    walk_phase("右侧中间(1050,650)", RIGHT_MID, TARGET_KEY2, 3)
print(f"  段2结束地图: '{get_map_name()}'")
print(f"  累计战斗: {battle_count} 场")
print("=== Phase 6 完成 ===")
