"""铁3副本(隐龙)任务 —— 步骤0传送 + 焚火废墟/烈焰野地/龙隐秘地/龙啸古地 多地图流转

断点续跑: 每个 Phase 完成后写状态文件 logs/dungeon_tie3_state_{serial}.json
...
# 走路传送门 (检测地图名变化即停)
POS_LEFT = (550, 200)            # 左上方(焚火废墟→烈焰野地)
POS_MID = (700, 200)             # 中间
POS_RIGHT = (1050, 200)          # 右上方(焚火废墟→烈焰野地)
POS_EDGE = (20, 600)             # 左边缘(烈焰野地→龙隐秘地 / 龙隐秘地→龙啸古地)
...
# 自动遇怪计数器 ("自动遇怪剩XX场" 在战斗界面下方)
COMBAT_COUNTER = (500, 1250)
COUNTER_RANGE = 150
...
# 阶段总数
TOTAL_PHASES = 15
MAX_BATTLES = 2                  # 交任务前需累计的普通战斗场数


class DungeonTie3Task(BaseTask):
    """铁3副本(隐龙)自动任务 — 16 Phase 完整流程"""
    ...
    def _screenshot_arr(self) -> np.ndarray:
        ...
        tmp = os.path.join(LOG_DIR, f"_dungeon_tie3_tmp_{os.getpid()}.png")
        adb_args = [adb]
        if self._serial:
    ...
    def _step0_memo_enter(self):
        self.log_key("── 步骤0: 备忘→副本→OCR找隐龙→瞬间传送 ──")
        self._tap(MEMO, "备忘", 1.2)
        self._tap(DUNGEON_TAB, "副本标签", 1.2)
        pos = self._find_text_in_area("隐龙", 300, 1300)
        if pos is None:
            pos = self._find_text_in_area("隐", 300, 1300)
        if pos is None:
            self.log_key("  [失败] 未找到隐龙行")
            return False
        self.log_key(f"  找到隐龙 @ {pos}")
    ...
    def _auto_battle_phase(self, count=2):
        ...
        while battles < count and time.time() - t0 < 300 and self._running:
            ...
            time.sleep(0.3)
        # 确保战斗已结束再按0取消（0是切换键：按1次开，再按1次关；落在战斗界面无效）
        if self._is_in_battle() and self._running:
        ...
        if self._check_text_at("剩", COMBAT_COUNTER, COUNTER_RANGE):
    ...
    def _find_npc(self, target):
        for page in range(1, 5):
            ...
            reader = self._get_reader()
            try:
                results = reader.readtext(arr, mag_ratio=1)
            except Exception:
                results = []
            for r in results:
                bbox, text, conf = r
    ...
    def _quest_teleport(self):
        self.log_key("── 任务传送: 选中(副)天星子大计→确定→瞬间传送 ──")
        if self._is_in_battle():
            self.log_key("  传送前遇怪! 等待战斗结束...")
            self._wait_battle_end()
        self._tap(KEY1, "任务列表(1)", 1.5)
        # "天星子大计"字体特殊OCR识别不出，改用"副本任务"锚点定位第二行
        ...
        if anchor is None:
            ...
            return False
        cx, cy = anchor
        row_y = cy + 130   # 天星子大计在副本任务下方第二行
        self.log_key(f"  锚点副本任务@({cx},{cy})，点击第二行天星子大计@({cx},{row_y})")
    ...
    def _state_file(self) -> str:
        ...
        return os.path.join(LOG_DIR, f"dungeon_tie3_state_{safe}.json")
    ...
    def run(self):
        ...
        # Phase 0: 步骤0 传送 (备忘→副本→隐龙→瞬间传送)
        if start_phase <= 0:
            self._log_phase(0, "传送(备忘→副本→隐龙→瞬间传送)")
        ...
        # Phase 2: 焚火废墟 遇怪1场
        if start_phase <= 2:
            self._log_phase(2, "焚火废墟 遇怪1场")
            self._auto_battle_phase(1)
            self._save_progress(2)

        # Phase 3: 焚火废墟 说书人→交→接 (第1轮)
        if start_phase <= 3:
            self._log_phase(3, "焚火废墟 说书人→交→接(第1轮)")
            if self._npc_phase("说书人"):
                self._submit_quest()
                self._battle_count = 0
                self._accept_quest()
        ...
        # Phase 4: 焚火废墟 遇怪1场
        if start_phase <= 4:
            self._log_phase(4, "焚火废墟 遇怪1场")
            self._auto_battle_phase(1)
            self._save_progress(4)

        # Phase 5: 焚火废墟 说书人→交→接 (第2轮)
        if start_phase <= 5:
            self._log_phase(5, "焚火废墟 说书人→交→接(第2轮)")
            if self._npc_phase("说书人"):
                self._submit_quest()
                self._battle_count = 0
                self._accept_quest()
        ...
        # Phase 6: 左上角(550,200)3次 → 右上角(1050,200)3次 → 烈焰野地
        if start_phase <= 6:
            self._log_phase(6, "走路→烈焰野地")
            if not self._walk_phase("左上角(550,200)", POS_LEFT, lambda n: "烈焰" in n, 3):
                self._walk_phase("右上角(1050,200)", POS_RIGHT, lambda n: "烈焰" in n, 3)
            self._save_progress(6)

        # Phase 7: 点(20,600)→龙隐秘地
        if start_phase <= 7:
            self._log_phase(7, "点(20,600)→龙隐秘地")
            self._walk_phase("左边缘(20,600)", POS_EDGE, lambda n: "龙隐" in n, 3)
            self._save_progress(7)

        # Phase 8: 龙隐秘地 找龙女→交→接
        if start_phase <= 8:
            self._log_phase(8, "龙隐秘地 找龙女→交→接")
            if self._npc_phase("龙女"):
                self._submit_quest()
                self._accept_quest()
        ...
        if start_phase <= 9:
            self._log_phase(9, "龙隐秘地 Boss战→交→接")
            self._boss_battle()
            self._submit_quest()
            self._accept_quest()
            self._save_progress(9)

        # Phase 10: 点(20,600)→龙啸古地
        if start_phase <= 10:
            self._log_phase(10, "点(20,600)→龙啸古地")
            self._walk_phase("左边缘(20,600)", POS_EDGE, lambda n: "龙啸" in n, 3)
            self._save_progress(10)

        # Phase 11: 龙啸古地 说书人→交→接 (第1次)
        if start_phase <= 11:
            self._log_phase(11, "龙啸古地 说书人→交→接(第1次)")
            if self._npc_phase("说书人"):
                self._submit_quest()
                self._battle_count = 0
                self._accept_quest()
        ...
        if start_phase <= 14:
            self._log_phase(14, "龙啸古地 Boss战→交→接")
            self._boss_battle()
            self._submit_quest()
            self._accept_quest()
        ...
        if start_phase <= 15:
            self._log_phase(15, "任务列表选中天星子大计→传送→提交")
            self._quest_teleport()
            self._submit_quest()
            self._save_progress(15)

        self._clear_progress()
        self.log_key("铁3副本流程完成!")
