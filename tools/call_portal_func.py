# -*- coding: utf-8 -*-
"""Hook portal functions to understand how to call them directly"""
...
with open(os.path.join(script_dir, 'call_portal_func.js'), 'r', encoding='utf-8') as f:
    js_template = f.read()
...
def on_msg(msg, data):
    ...
    if ptype == 'ready':
        ...
        print(f"[!] {payload['msg']}", flush=True)
    elif ptype == 'portal_ctx':
        print(f"\n>>> PORTAL context:", flush=True)
        print(f"    x0={payload['x0']} x1={payload['x1']}", flush=True)
        print(f"    x2={payload['x2']} x3={payload['x3']}", flush=True)
        print(f"    lr={payload['lr']} sp={payload['sp']}", flush=True)
    elif ptype == 'c20341':
        print(f"  [0x20341] a0={payload['a0']} a1={payload['a1']} a2={payload['a2']} a3={payload['a3']}", flush=True)

script = session.create_script(JS)
script.on('message', on_msg)
script.load()

print("[*] 走传送门，抓参数... 120秒", flush=True)
try:
    for i in range(120):
        time.sleep(1)
