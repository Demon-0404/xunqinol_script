...
with open(os.path.join(script_dir, 'diag_packet.js'), 'r') as f:
    js_template = f.read()
...
def on_msg(msg, data):
    ...
    ptype = payload.get('t', '?')
    if ptype == 'ready':
        print("[*] 诊断就绪，请点击移动角色...", flush=True)
    elif ptype == 'send':
        print(f"[SEND#{payload['n']}] len={payload['len']} first=0x{payload['first']:02x} | {payload['hex']}", flush=True)
