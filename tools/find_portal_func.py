# -*- coding: utf-8 -*-
"""Find the game function that constructs portal packet via call stack"""
...
with open(os.path.join(script_dir, 'find_portal_func.js'), 'r') as f:
    js_template = f.read()
...
def on_msg(msg, data):
    ...
    if ptype == 'ready':
        ...
        print(f"\n[PORTAL] len={payload['len']} — 调用栈:", flush=True)
    elif ptype == 'frame':
        print(f"  #{payload['n']} {payload['mod']}+{payload['off']} ({payload['addr']})", flush=True)
    elif ptype == 'stack_end':
        print("---", flush=True)
...
print("[*] 请走过传送门... 60秒后自动退出", flush=True)
try:
    for i in range(60):
        time.sleep(1)
