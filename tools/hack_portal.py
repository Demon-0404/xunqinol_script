# -*- coding: utf-8 -*-
"""Hook all 3 libtcb functions + send() to trace portal flow"""
...
with open(os.path.join(script_dir, 'hack_portal.js'), 'r', encoding='utf-8') as f:
    js_template = f.read()
...
def on_msg(msg, data):
    ...
    if ptype == 'info':
        ...
        print(">>> PORTAL PACKET DETECTED <<<", flush=True)
    elif ptype == 'c6d':
        print(f"  [0x6d289 #{payload['n']}] a0={payload['a0']} a1={payload['a1']} a2={payload['a2']}", flush=True)
    elif ptype == 'c65':
        print(f"  [0x653eb] a0={payload['a0']}", flush=True)
    elif ptype == 'c20':
        print(f"  [0x20341] a0={payload['a0']} a1={payload['a1']} a2={payload['a2']}", flush=True)

script = session.create_script(JS)
script.on('message', on_msg)
script.load()

print("[*] 请走过传送门... 等待60秒", flush=True)
try:
    for i in range(60):
        time.sleep(1)
