# -*- coding: utf-8 -*-
"""Inject captured map RECV data back into the game"""
...
with open(os.path.join(script_dir, 'inject_map.js'), 'r', encoding='utf-8') as f:
    JS = f.read() % game_fd
...
pkt0_hex = captured[0]['hex']
print(f"Loaded captured data: {captured[0]['len']} bytes", flush=True)
...
def on_msg(msg, data):
    ...
    ptype = payload.get('t', '?')
    if ptype == 'ready': print("[*] Injector ready", flush=True)
...
print("=" * 50, flush=True)
print("WARNING: Injecting map data. May crash or disconnect!", flush=True)
print("Injecting in 5 seconds...", flush=True)
time.sleep(5)

print(f"Injecting {captured[0]['len']} bytes of map data...", flush=True)
result = script.exports_sync.inject(pkt0_hex)
print(f"Result: {result}", flush=True)

print("Waiting 15s... observe game behavior!", flush=True)
time.sleep(15)
