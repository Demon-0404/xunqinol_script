# -*- coding: utf-8 -*-
"""Sniff all outgoing packets to find map transition/teleport packet"""
...
with open(os.path.join(script_dir, 'sniff_all.js'), 'r') as f:
    js_template = f.read()
...
# Store all packets for analysis
packets = []
...
def on_msg(msg, data):
    ...
    if ptype == 'ready':
        ...
        h = payload['hex']
        dec = payload.get('dec', '')
        print(f"[#{n}] dt={dt}ms len={l} | {h}{dec}", flush=True)
        packets.append(payload)

script = session.create_script(JS)
script.on('message', on_msg)
script.load()
print("[*] Recording... 走过传送门后等待5秒，自动停止", flush=True)
try:
    for i in range(60):
        time.sleep(1)
...
log_path = os.path.join(script_dir, '..', 'logs', 'sniff_all.txt')
os.makedirs(os.path.dirname(log_path), exist_ok=True)
with open(log_path, 'w') as f:
    for p in packets:
        f.write(f"[#{p['n']}] dt={p['dt']}ms len={p['len']} | {p['hex']}{p.get('dec', '')}\n")
